...
def _build_input_df() -> pd.DataFrame:
    """Build deterministic two-row input used by pipeline audit tests."""
...
def _stub_pipeline_steps(monkeypatch, input_df: pd.DataFrame) -> None:
    """Stub transformation steps so tests focus only on audit persistence behavior."""
...
def _assert_perf_attrs(result_df: pd.DataFrame) -> None:
    """Assert the expected performance attributes are attached to pipeline output."""
...
def test_run_pipeline_exposes_performance_attrs_and_writes_log(
    ...
    _stub_pipeline_steps(monkeypatch, input_df)
